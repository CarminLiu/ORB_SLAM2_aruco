...
{
    cout<<"in Optimizer::BundleAdjustment"<<endl;
...
//!!!!!!!!!!!
    cout<<"Set MapAruco vertices"<<endl;
    int nMonoEdge = nnedges;
    int wei = 0;
    if(vpMA.size()>0){
        wei = nMonoEdge/vpMA.size();
        wei /= 4;
        if(wei>50) wei=50;
    }
    wei = 3;
    ...
    for(size_t i=0; i<vpMA.size(); i++)
    {
        ...
        for(map<KeyFrame*, size_t>::const_iterator mit=mapObs.begin(), mend=mapObs.end(); mit!=mend; mit++)
        {
            ...
            {
                g2o::EdgeMarker *e = new g2o::EdgeMarker();
                ...
                Eigen::Matrix<double,2,1> obs;
                obs<<pKFi->mvArucoUn[4*idx+k].x, pKFi->mvArucoUn[4*idx+k].y;
                e->setMeasurement(obs);
                e->setInformation(Eigen::Matrix2d::Identity()*3);
...
int Optimizer::PoseOptimizationByAruco(Frame *pFrame)
{
    // cout<<"Optimizer::PoseOptimizationByAruco(Frame *pFrame)"<<endl;
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
    ...
    // Set Frame vertex
    g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
    cv::Mat pFrametcw = pFrame->mTcw.rowRange(0,3).col(3); //* Record
    vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
    vSE3->setId(0);
    vSE3->setFixed(false);
    optimizer.addVertex(vSE3);
    ...
    if(Frame::mbUArucoIni){
    int nMonoEdge = vpEdgesMono.size();
    int wei = 0;
    if(pFrame->NA > 0){
        wei = nMonoEdge/pFrame->NA;
        wei /= 4;
        if(wei>50) wei=50;
    }
    // wei *= 0.8;
    wei = 3;
    // cout<<"pose optimization wei = "<<wei<<endl;
         
    {
        unique_lock<mutex> lock(MapAruco::mGlobalMutex); 
        unsigned long maxArucoId = 0;
        for(size_t i=0; i<pFrame->NA; i++) 
        {
            if(pFrame->mvbOldAruco[i]==true){
                // cout<<pFrame->mvMarkers[i].id<<"\033[31m: is an old aruco\033[0m"<<endl;
                continue;
            }
            MapAruco* pMA = pFrame->mvpMapArucos[i];//! 这里没有考虑以后会出现的回环问题，而是将见到的Aruco全用。
            if(pFrame->mvbArucoGood[i]==false)
                continue;
            if(pMA)
            {
                // if(pFrame->mnId >450 && pMA->GetFirstFrameid() + 900 < pFrame->mnId) //* 说明是很久以前见到的Aruco， 900/30Hz = 30s
                //     continue;
                ...
                for(size_t k=0; k<4; k++)
                {
                    ...
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(id)));
                    Eigen::Matrix<double,2,1> obs;
                    obs<<pFrame->mvArucoUn[4*i+k].x, pFrame->mvArucoUn[4*i+k].y;
                    e->setMeasurement(obs);
                    e->setInformation(Eigen::Matrix2d::Identity()*3);
                    auto p = pMA->get3DPointsLocalRefSystem(k);
    ...
    // cout<<"set mapAruco verteces"<<endl;
    ...
    // cout<<"in Optimize #MPs: N = "<<N<<"; #MONOedge = "<<vpEdgesMono.size()<<"; nBad = "<<nBad<<endl; 

    // Recover optimized pose and return number of inliers
    g2o::VertexSE3Expmap* vSE3_recov = static_cast<g2o::VertexSE3Expmap*>(optimizer.vertex(0));
    g2o::SE3Quat SE3quat_recov = vSE3_recov->estimate();
    cv::Mat pose = Converter::toCvMat(SE3quat_recov);
    cv::Mat poset = pose.rowRange(0,3).col(3);
...
{    
    ...
    // cout<< "---------------BEGIN OF LOCAL BA----------------------" <<endl;
    list<KeyFrame*> lLocalKeyFrames;
    vector<int> viSaveKFid;

    lLocalKeyFrames.push_back(pKF);
    pKF->mnBALocalForKF = pKF->mnId;
    viSaveKFid.push_back(pKF->mnId); //! Add by liujiamin
    ...
    for(int i=0, iend=vNeighKFs.size(); i<iend; i++)
    {
        ...
        if(!pKFi->isBad())
        {
            ...
            viSaveKFid.push_back(pKFi->mnId); //! Add by liujiamin
        }   
    }

    // Local MapPoints seen in Local KeyFrames
    list<MapPoint*> lLocalMapPoints;
    map<int, vector<MapPoint*> > mMapPointsInAruco;
            ...
            if(pMP)
                if(!pMP->isBad())
                    if(pMP->mnBALocalForKF!=pKF->mnId)
                    {
                        ...
                        pMP->mnBALocalForKF=pKF->mnId; //! Add by liujiamin
                        if(pMP->forflag == 1){
                            int pid = pMP->mArucoID;
                            mMapPointsInAruco[pid].push_back(pMP);
                        }
                    }
        }
    }
    ...
    vector<MapAruco*> vLocalMapArucosMono;
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin() , lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        {
            if((*lit)->isIdxArucoOld(i)==true )
            {
                // cout<<(*lit)->mvMarkers[i].id<<"\033[32m: is an old aruco\t (*lit)->mvMarkers\033[0m"<<endl;
                continue;
            }
            ...
            if(pMA && !pMA->isBad())
            {
                ...
                {
                    vLocalMapArucosMono.push_back(pMA);
    ...
    std::map<int, double>& ArucoMeanLength = pMap->mArucoMeanLength;
   ...
   //* 存最大地图点的ID号
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        //! 因为这里加了MapAruco，所以与需要一个变量去存最大的ID号
        if(id > maxMPid)
        {
            maxMPid = id;
        }
    ...
    // TODO: Set Edge EdgeMapPointInAruco
    // for(map<int, vector<MapPoint*>>::iterator mit=mMapPointsInAruco.begin(), mend=mMapPointsInAruco.end(); mit!=mend; mit++)
    // {
    //     // cout<<"g2o::EdgeMapPointInAruco add"<<endl;
    //     int idf = mit->first;
    //     int idRecord = -1;
    //     for(int i=0; i<viSaveArucoId.size(); i++)
    //     {
    //         if(viSaveArucoId[i] == idf) {
    //             idRecord = i;
    //             break;
    //         }
    //     }
    ...
    //     vector<MapPoint*> vMP = mMapPointsInAruco[idf];
    //     MapAruco* pMA = vLocalMapArucosMono[idRecord];
    //     cv::Mat r = pMA->GetRw2m();
    //     cv::Mat t = pMA->Gettw2m();

    //     for(size_t i=0; i<vMP.size(); i++) 
    //     {
    //         MapPoint* pmp = vMP[i];
    //         g2o::EdgeMapPointInAruco* e = new g2o::EdgeMapPointInAruco(r, t);
    //         int idmp = pmp->mnId+maxKFid+1;
    //         double obs = 0.0;
    //         e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(idmp)) );
    //         e->setMeasurement(obs);
    //         // e->setInformation(1);
    //         optimizer.addEdge(e);
    ...
    //     }
    // }
    // cout << "vpEdgesMono.size = " << vpEdgesMono.size();
    // if(Frame::mbUArucoIni){
    // int cnt_aruco_edge = 0;
    ...
    // }
    // // cout<<"\tvLocalMapArucosMono.size = "<<vLocalMapArucosMono.size() << "\tcnt_aruco_edge = "<<cnt_aruco_edge<<endl;
    // int nMonoEdge = vpEdgesMono.size();
    // int wei = 0;
    // if(cnt_aruco_edge > 0){
    //     wei = nMonoEdge/cnt_aruco_edge;
    //     wei /= 4;
    //     if(wei>50) wei=50;
    // }
    // // wei *= 0.8;
    ...
    // {
    //     MapAruco* pMA = *vit;
    //     int plid = pMA->GetPlane()->GetID();
    //     vector<int>::iterator viit = find(viSavePlaneId.begin(), viSavePlaneId.end(), plid);
    //     if(viit == viSavePlaneId.end())
    //     {
    ...
    cout<<"iterative optimization THE FIRST TIME "<<endl;
    optimizer.initializeOptimization();
    ...
    // // TODO: check MapAruco inlier observations
    // for(size_t i=0; i!=vpEdgePlaneMarker.size(); i++)
    // {
    //     g2o::EdgePlaneMarker* e = vpEdgePlaneMarker[i];
    //     if(e->chi2()>5.991)
    //         e->setLevel(1);
    //     e->setRobustKernel(0);
    // }
    ...
    cout<<"iterative optimization THE ==SECOND== TIME "<<endl;
    optimizer.initializeOptimization(0);
    ...
    cout<<vLocalMapArucosMono.size()<<endl;
    int cntma=0;
    for(vector<MapAruco*>::iterator vit=vLocalMapArucosMono.begin(), vend=vLocalMapArucosMono.end(); vit!=vend; vit++)
    {
        ...
        g2o::SE3Quat SE3quat = vSE3Aruco->estimate();
        cv::Mat T = Converter::toCvMat(SE3quat);
        double diff = cv::norm( T.rowRange(0,3).col(3) - pTma.rowRange(0,3).col(3));
        if( diff<1e-5){
            // cout<<"\033[31m--- ---  ===OPTIMIZE=== --- --- the same \n \033[0m"<<endl;
            cntma++;
        }
        cv::Mat R = T.rowRange(0,3).colRange(0,3);
        cv::Mat t = T.rowRange(0,3).col(3);
    ...
    // cout<<"vLocalMapPlanes.size() = "<<vLocalMapPlanes.size()<<endl;
    // int cntmpl=0;
    // for(vector<MapPlane*>::iterator vit=vLocalMapPlanes.begin(), vend=vLocalMapPlanes.end(); vit!=vend; vit++)
    // {
    //     MapPlane *pMPL = *vit;
    //     g2o::VertexPlane* vPL = static_cast<g2o::VertexPlane*>(optimizer.vertex(pMPL->GetID()+maxMAid+1));
    //     g2o::Plane3D p3d = vPL->estimate();
    //     Eigen::Vector4d p3dCoeff = Converter::toVector4d(p3d);
    //     double diff2 = (pMPL->GetCoeffs()-p3dCoeff).norm();
    //     if(diff2<1e-5){
    //         // cout<<"\033[32m--- ---  ===OPTIMIZE=== --- --- the same \n\033[0m"<<endl;
    //         cntmpl++;
    //     }
        
    //     pMPL->SetCoeffs(p3dCoeff);
    // }
    // cout<<"cntmpl = "<<cntmpl<<endl;
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    // set<int> sAId;
    set<int> sModified;
    unordered_map<int, int> mARelatedKF;
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        // TODO 修正Aruco的位置
        if(pMP->forflag){
            int ArucoId = pMP->mArucoID;
            set<int>::iterator sit = sModified.find(ArucoId);
            if(sit==sModified.end()){
                KeyFrame* pTmp;
                if(pMP->mnCorrectedByKF==pCurKF->mnId){
                    pTmp = pCurKF;
                }
                else 
                    pTmp = pMP->GetReferenceKeyFrame();
                
                int idx = pTmp->mmArucoIdandIdx[ArucoId];
                MapAruco* pMA = pTmp->GetMapAruco(idx);
                if(pMA && !pMA->isBad() )
                {
                    pMA->UpdateTcmByKF(pTmp, idx);
                    pMA->SetRtwmByKeyFrame(pTmp->GetRotation().t(), pTmp->GetCameraCenter());
                }

                sModified.insert(ArucoId);
            }
        }
