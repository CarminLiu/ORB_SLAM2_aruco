...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mvpMapArucos(F.mvpMapArucos), NA(F.NA),
    mvMarkers(F.mvMarkers), mvpArucoMPs(F.mvpArucoMPs), mvArucoUn(F.mvArucoUn), mvbArucoGood(F.mvbArucoGood),
    DoneScale(false), mmArucoIdandIdx(F.mmArucoIdandIdx), mvbOldAruco(F.mvbOldAruco)
{
    mnId=nNextId++;
...
void KeyFrame::EraseMapArucoMatch(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpMapArucos[idx]=static_cast<MapAruco*>(NULL);
}

bool KeyFrame::isIdxArucoOld(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mvbOldAruco[idx]; //!!!!!!!!!!!!!!!!!!!!!!!! 记得回环形成之后更改
}

void KeyFrame::SetIdxArucoOldFalse(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvbOldAruco[idx] = false;
}
...
{   
    ...
    //* add about erase MapAruco's Observation
    for(size_t i=0; i<mvpMapArucos.size(); i++)
        if(mvpMapArucos[i])
            mvpMapArucos[i]->EraseObservation(this);
