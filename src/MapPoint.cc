...
MapPoint::MapPoint(const cv::Mat &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    nArucoObs(0), forflag(0), mArucoID(-1), DoneScale(false)
{
...
void MapPoint::AddArucoObservation(KeyFrame* pKF, size_t idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    if(mArucoObservations.count(pKF)) return;
    mArucoObservations[pKF]=idx;
    nArucoObs++;
}
